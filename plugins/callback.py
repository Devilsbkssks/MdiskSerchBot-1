...
async def dbgive_access_handler(c:Client,query: CallbackQuery):
    try:
        ...
        return await c.send_message(from_user, f"Your channel {db_channel} has been verified. @AM_ROBOTS will search posts from your channel only in your group")
    except Exception as e:
        print(e)
...
async def dbdeny_access_handler(c:Client,query: CallbackQuery):
    ...
    return await c.send_message(from_user, f"Your request for channel [`{db_channel}`] has been rejected by @AM_ROBOTS Please Contact admin For More Information")
...
async def button(bot, cmd: CallbackQuery):
        ...
        if "About_msg" in cb_data:
            ...
            InlineKeyboardButton('➕ Add Me To Your Groups ➕', url=f'http://t.me/{Config.BOT_USERNAME}?startgroup=true')
                        ...
                        InlineKeyboardButton('Watch Tutorial', url='https://www.youtube.com/@Anuragtechnical')
                    ]
                ]
            ),
            parse_mode="html"
